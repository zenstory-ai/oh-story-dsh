...
提取以语义变化为边界：风险、信息、关系、资源、决定、行动或读者理解发生变化时记一个节点；同一动作链连续完成多个功能时可记为一个节点并在描述中说明，不按字数或小节配额拆分。覆盖全文真实变化，不为达到目标数量重复记录同一事件。
...
从情节节点数据生成情感曲线。只在情绪立场、期待或压力实际变化处取点；完整任务、推理、手艺或等待链没有独立情绪转折时无需为该节补点。
...
5. **高度浓缩**：一个节点一句话概括
6. **复合合并**：为同一情绪/叙事目的服务的连续微动作合并为一个节点，共用一个 N 编号；统计「节点数」时按最终 N 编号总数计
...
| 指标 | 计算方式 | 说明 |
|------|----------|------|
| 对话占比 | 对话字数 / 全文字数 | 描述该文本，并与同题材、同平台样本对照；不作通用合格线 |
| 潜台词率 | 含潜台词对话条数 / 总对话条数 | 描述人物回避与信息控制；直说在告知、求救、立规矩等场景也可成立，不把越高当越好 |
| 审判式对话 | 主角提问→对方自曝的数量 | 记录提问是否迫使选择、暴露矛盾或改变关系，不设最低处数 |
...
| 维度 | 计算方式 | 对照方式 | 说明 |
|------|----------|----------|------|
| 事件密度 | 节点数 / 全文字数 × 1000 | 同题材、同平台样本及本文前后分布 | 结合节点是否真实改变状态判断，不用数量直接判水或赶 |
| 对话密度 | 对话字数 / 全文字数 × 100% | 同题材、同平台样本 | 只描述文本倾向；对白功能与无对白场景的推进方式另行判断 |
| 冲突密度 | 冲突节点数 / 总节点数 × 100% | 同题材、同平台样本及本文压力来源 | 任务、证据、等待、空间和关系压力须另行记录，不用直接冲突占比代替张力判断 |
| 信息密度 | 信息节点数 / 总节点数 × 100% | 本文读者模型变化与后果篇幅 | 判断新信息是否可理解、会改写判断并得到消化，不设通用比例 |

### 节奏异常检测

| 异常模式 | 文本信号 | 复核问题 |
|----------|----------|----------|
| 节奏塌陷 | 连续叙事单元没有状态变化、因果后果或读者模型更新 | 场景是在蓄力、完成任务，还是原地复述？ |
| 压力过载 | 多次升级之间没有选择、损失、反应或后果 | 读者是否来得及理解每次升级改变了什么？ |
| 信息洪峰 | 新事实连续出现，却没有查证、定位、行动或旧线索改义 | 哪些信息真正改变判断，哪些只是清单？ |
| 对白失效 | 对话可整段删除，或只剩问答、复述和设定讲解 | 每轮是否改变信息、策略、权力、关系或行动？ |
| 对白挤压场景 | 人物只说不做，空间、任务和后果消失 | 是否需要动作或留白承接，而不是按占比截断？ |
| 反转过载 | 新反转在上一层尚未产生选择或后果时就将其抹掉 | 每层是否有证据、读者模型变化和可感后果？ |
...
- **source-story-quality.md** = 短篇拆书的质量自检清单（评估**源文**质量时用，数值引用本文件）
- **analysis-report-style.md** = 拆文报告**本身**的证据链与表达守门员（扫描 `拆文报告.md`，不是源文滤镜）
