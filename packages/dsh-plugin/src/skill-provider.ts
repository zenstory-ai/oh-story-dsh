...
const DSH_DRAMA_OVERRIDES: Readonly<Partial<Record<string, string | ((skillRoot: string) => string)>>> = {
  ...
  "short-drama-edit": [
    ...
    "Every cut must share one width, height and frame rate: render hard-joins the encoded segments and never scales or retimes them. When check reports a mismatch, normalise the odd clips to the delivery spec with ffmpeg in the DSH execution world, under approval, preserving the composition and writing new files beside the originals — never overwrite produced footage. Record the crop or pad in that CUT's block (not in its 画面 line, which accepts only 亮度/饱和/色温), point 来源 at the new file, update 入点/出点 if the conversion moved them, and rerun check.",
