...
猜，也不自行扩充新的禁用类别。时间线总结只列本镜实际有的声音层。
   无对白镜写清底声或静默及其覆盖区间；嘴部动作按剧本表演，不默认强制闭嘴。
   非画内配乐按目标方言声明，有需要时明确写空。
13. 成稿时检查动作、声音和限制条件是否清楚且相容。正面状态有助于表达目标，必要的否定条件
   可以保留；不按否定句数量判错。确认不跨下一镜、不要求互斥动作、不让人物或道具瞬移变形。
