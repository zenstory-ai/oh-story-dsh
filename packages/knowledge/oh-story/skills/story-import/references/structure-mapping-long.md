...
旧拆文的目录或字段名与上表不同时，先建立一次“旧路径 → 当前目标”的来源映射，再按实际可读内容转换；不要为了改名复制出第二套拆文真源。`剧情/节奏.md`、`剧情/情绪模块.md` 或主题化设定缺失时，旧内容能支持的导入步骤照常执行，对应摘要写「未知/待增强」；只有用户要启用依赖这些资料的新对标召回时，才回到 story-long-analyze 做按需增强。
...
| 摘要字段 | 细纲字段 | 转换方式 |
|---------|---------|---------|
| 关键事件 + 局面结果 | 核心事件 | 合并为“谁采取什么行动 → 局面如何变化”；旧摘要没有 `局面结果` 时只用 `关键事件` |
| 章节字数 | 字数目标 + 字数口径 | 对原文章节运行 `storyctl.py wordcount measure`，写入 `actual` 与 `visible_chars_v1` |
| `基调：` | 目标情绪 | 读取紧张/轻松/悲伤/热血/爽/甜/温馨/恐怖/压抑/其他枚举；缺失写 `[待补充]` |
| 章尾钩子 | 章首承接证据 | 作为下一章开篇需要承接的等待；新的章首钩子设计目标仍标 `[待补充]` |
| 爽点类情节点 | 爽点 | 从情节点类型推断；没有则写“无显性爽点 / [待补充]” |
| 情节点起承转合 + 因果 | 内容概括（起因/发展/转折/高潮/结尾） | 按情节点顺序归纳，用因果链校正起因与转折；证据不足写 `[待补充]` |
| 主线/支线/任务线索 | 情节安排（主线/辅线/事件线/感情线/逻辑线） | 从剧情单元索引与摘要反推；无证据的辅线/感情线写“无”或 `[待补充]`，不得杜撰 |
| 涉及 + 状态变化 | 人物关系和出场顺序 | `涉及` 给出候选人物，`状态变化` 只写有证据的“前 → 后”；关键物件从证据或旧情节点补充，缺失写 `[待补充]` |
| 全部情节点 | 情节细化 / 情节点序列 | 按表格逐行写（# / 情节点 / 功能标签 / 执行边界）；保持原顺序，功能或边界不明写 `[待补充]`，不反推逐点字数配额 |
| 胜负/反转/收益损失 | 行动成本（可无）/收益归属 | 有明确证据才填写；行动成本可无、不硬造；否则 `[待补充]` |
| 章尾钩子 + 局面结果 | 结尾设定和钩子 | 局面结果写收束状态，章尾钩子写已存在的等待；未来设计仍标 `[待补充]` |
...
| `拆文库/{对标书名}/章节/*.md`（第N章_摘要.md + 黄金三章 第1-3章_深度拆解.md）| `{项目}/对标/{对标书名}/章节/*.md` | 匹配章证据；摘要读取概要、关键事件、局面结果、涉及、状态变化与情节点序列（含类型、主题标签与基调），黄金三章另含深度写法 |
