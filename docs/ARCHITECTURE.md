...
| Creative file tree, editor, production projection and file following | `@oh-story/dsh` Browser contribution |
...
5. The side-effect-free `oh_story_production` UI-intent tool for semantic production focus, explicit sequence order and Agent-owned job projection. Cosmetic canvas layout remains creator-controlled Session state.
6. `tools/pre-execute` and `tools/post-execute` hooks for long-form outline and Tracking checks.
7. A Session-scoped creative file route for Markdown, text, JSON, JSONL and previewable production media.
