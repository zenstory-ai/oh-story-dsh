...
配置好模型、选中正确 workspace 后，可以先用下面的自然语言请求，而不立即导入或改稿。替换方括号内容后再发送；先把创作方向谈清楚，再决定是否落到项目文件。
...
这轮是创作讨论，不是项目导入：只在 Chat 回复不会建立小说目录或自动显示新的工作台。选定方向后，再明确要求按随包流程导入或规划，并把“是否写文件、是否写正文、写到哪一章”说清楚；保留原稿备份。
