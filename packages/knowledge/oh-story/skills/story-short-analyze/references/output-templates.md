...
> 提取方法论与节点边界详见 material-decomposition.md「情节节点提取规则」（唯一权威）。
...
至少 5 个节点，取点落在情绪立场、期待或压力实际变化处，含字数位置，每个节点标注钩子类型：
...
> 对话占比只描述该文本。判断须引用具体场景功能，并与同题材、同平台对标样本比较；不套通用范围。

| 指标 | 数值 | 评价 |
|------|------|------|
| 对话占比 | {X%} | {该文本倾向/相对对标偏高/相对对标偏低，并说明场景功能} |
| 潜台词率 | {X%} | {结合告知/求救/试探/回避等具体场景说明效果，不以越高越好} |
...
| 指标 | 数值 | 判断 |
|------|------|------|
| 事件密度 | {X个/千字} | {文本分布与真实状态变化} |
| 对话密度 | {Y%} | {文本倾向、对白功能与无对白推进方式} |
| 冲突密度 | {Z%} | {直接冲突与其他压力来源的关系} |
...
> 各维度数值只作文本描述。判断须回到具体节点、状态变化和后果，并在存在对标时与同题材、同平台样本比较。

| 维度 | 数值 | 判断 |
|------|------|------|
| 事件密度 | {N}个/千字 | {文本分布与真实状态变化} |
| 对话密度 | {X%} | {文本倾向、对白功能与无对白推进方式} |
| 冲突密度 | {X%} | {直接冲突与任务/证据/空间/关系压力的关系} |
| 信息密度 | {X%} | {读者模型变化与消化效果} |
...
Stage 完成前逐项检查，缺一即不完整。**节点密度、对话占比等只作结构化描述；质量判断须引用具体场景功能、状态变化和同题材对标，不设通用数值阈值。**
...
- [ ] POV 已识别 `[WARN]`
- [ ] 叙事时间线已标注 `[WARN]`
- [ ] 情节节点覆盖全文真实变化，没有为凑数量重复记录同一事件（见 material-decomposition.md「情节节点提取规则」） `[BLOCK]`
- [ ] 每个节点有情绪标记（类型 + 强度值） `[BLOCK]`

**Stage 3（情感线+爆点）**：
- [ ] 至少 5 个情感节点，覆盖每一处情绪立场、期待或压力变化，每个有字数位置、强度值和方向（虐-9~爽+9） `[BLOCK]`
