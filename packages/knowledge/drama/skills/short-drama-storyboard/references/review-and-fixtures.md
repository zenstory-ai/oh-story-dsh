...
- 提示词条目与真实输入图片分开记录；只有创作者已提供或经确认生产、且当前可读的实际图片才能声称为「输入参考图」，并用独立稳定的 `REF-...` 槽位、显式顺序、仅用 `/` 分隔的项目相对路径、中文名称、用途、控制与不得控制范围定位；每个槽位的 `用途` 取自封闭词表，一镜最多一张起始帧与一张结束帧；一张都没有、只准备了一部分、全部就绪和创作者明确选择文生视频必须如实区分；
