...
- 最多做 2 轮定向 repair；仍失败则停止，用中文告诉作者设计还缺什么，不得声称 Phase 2 已完成
- exit 2、脚本缺失或不可执行：告诉作者自动检查没跑成，不得用泛化“自检”替代后继续
- 向作者汇报构思只讲故事（梗概、反转、付费点在第几节）和待拍板的问题（附推荐默认），不提脚本、检查 ID、字段或表格列数
