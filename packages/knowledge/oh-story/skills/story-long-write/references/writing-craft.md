...
- 同一场戏里的几个点可以合并、互相打断，不必排成逐点清单；因果依赖或刻意的节拍分隔才串行。只改顺序不会让正文变好看（交错与否的对照实验测不出差别），密点展开、疏点带过更要紧（见「疏密分配」）。
...
只许指出正文里**已有**的那一处，没有就答「无」，**不许为了让某一问有答案回头补内容**；只报告实际删改与保留理由，写手不另交验收说明。详写的关键场景里，遮住一处发生、感知或反应后读者什么也没损失，就删掉它；套式反应按第 1 节「套式反应删除测试」逐处过。
