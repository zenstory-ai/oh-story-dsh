...
function ShotBoard(props: Props & { readonly onCreateJob: (targetId: string, kind: "image" | "video", prompt: string) => Promise<void>; readonly onBatch: (kind: "image" | "video") => Promise<void> }) {
  return <section className="oh-story-shot-board"><div className="oh-story-production-actions"><button type="button" onClick={() => { void props.onBatch("image"); }}>准备批量关键帧</button><button type="button" onClick={() => { void props.onBatch("video"); }}>准备批量视频</button></div><div className="oh-story-shot-grid">{props.production.shots.map((shot) => {
    ...
    return <article className="oh-story-shot-card" data-selected={props.selectedId === shot.id || undefined} key={shot.id} onClick={() => { props.onSelect(shot.id); }}>
...
function VersionStrip({ targetId, versions, selections, onSelectionsChange }: { readonly targetId: string; readonly versions: readonly ProductionMediaVersion[]; readonly selections: Readonly<Record<string, string>>; readonly onSelectionsChange: (value: Record<string, string>) => void }) { const selectedByKind = new Map((["image", "video"] as const).map((kind) => [kind, selectedVersionForTarget(targetId, versions, selections, kind)?.id])); return <div className="oh-story-version-strip" aria-label={`${targetId} 成果版本`}>{versions.map((version, index) => <button type="button" aria-label={`选择 ${targetId} ${version.kind === "image" ? "图片" : "视频"}版本 ${String(index + 1)}`} data-selected={version.id === selectedByKind.get(version.kind) || undefined} key={version.id} onClick={(event) => { event.stopPropagation(); onSelectionsChange({ ...selections, [selectionKey(targetId, version.kind)]: version.id }); }}><MediaPreview version={version} interactive={false} /><span>V{String(index + 1)}</span></button>)}</div>; }
