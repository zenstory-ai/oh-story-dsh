...
- **输入参考图**：`REF-<slot>（顺序：<n>）· <项目相对路径>《<中文名>》（用途：<purpose>；控制：<may_control>；不得控制：<must_not_control>）`；`用途` 取 `身份|造型状态|地理|构图|尺度|效果|起始帧|结束帧|风格`；必须与分镜一致且全部就绪；只有创作者明确选择文生视频时可写「无（创作者已明确选择文生视频）」，无图或部分有图的「待补参考图」都不得进入最终视频提示词
