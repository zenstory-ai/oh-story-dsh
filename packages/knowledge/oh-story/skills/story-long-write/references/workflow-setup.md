...
1. 定位当前卷、已写进度与既有剧情单元卡：读 `大纲/大纲.md`；卷纲**走取段器不整读**（见 SKILL.md「卷纲不整读」）——先 `outline_view.py --toc {卷纲路径}` 看一屏目录定位当前单元，再 `--unit {单元ID}` 取闭包，新单元尚未建卡时先用 `--contract` 取卷级契约；另读 `追踪/上下文.md`（续写状态卡，待回收伏笔取 `## 活跃伏笔`、主角当前状态取 `## 核心角色状态`），以及 `设定/题材定位.md`（终局底牌与升级台阶）——续写状态卡缺项按 workflow-daily「旧信息查找步骤」按需查询，不读全部逐章增量；
