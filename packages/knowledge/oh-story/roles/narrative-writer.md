...
2. **展开子事件**：发生、感知、反应是可选信息维度，不是每段配额；关键子事件只把确有新信息的维度揉进同一镜头（详写的子事件合计 ≥100-150 字；过场/连接类 1-2 句带过，不要每个子事件平均用力，见 `story-setup/references/agent-references/writing-craft.md`「疏密分配」）
