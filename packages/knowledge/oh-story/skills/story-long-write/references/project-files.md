...
需落盘时在用户指定目录按下表组织；只讨论不建工程，仅规划只创建本次产物所需目录，不为凑齐目录树生成正文或追踪：
...
| 追踪/上下文.md（续写状态卡，≤12KB） | 全书当前状态 | 与 state 同时初始化 | 日更每章整份读；由事务工具整份重建，固定 7 栏 |
...
6. **公共灵感召回**（工作区有 `灵感库/` 时）按任务标签选 Top 3–8 张 active CBA；不传 IA/NM，不把灵感卡当风格样本；灵感卡只影响设计选择，与上述权威文件或细纲事实冲突时以后者为准。
