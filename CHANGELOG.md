...
- 「生产 · 任务」页新增投产通道就绪预检：未配置的 adapter 会在创作者确认之前列出所缺的环境变量。上游 adapter 只在 `run` 阶段读取凭据，而 `production_tool.py` 先消耗确认再启动 adapter，缺凭据时一次确认会被白白用掉。工作台只读取「是否已设置」，凭据取值不出宿主进程。`packages/dsh-plugin/README.md` 补充了完整的凭据与 Base URL 变量表（此前仓库中没有任何相关文档）。
